const state = {
  ...
  refreshMs: 3000,
  mediaBrowser: { libraryId: null, path: '', sort: localStorage.getItem('mediaBrowserSort') || 'estimatedSaveDesc' },
  timer: null,
  integrations: [],
  libraries: [],
...
function ensureMediaBrowserControls() {
    ...
    if (!librarySelect || document.getElementById('media-browser-sort')) return;
    const row = librarySelect.closest?.('.form-row') || librarySelect.parentElement;
    if (!row) return;

    const sortLabel = document.createElement('label');
    sortLabel.className = 'media-browser-sort-control';
    sortLabel.innerHTML = `Sort <select id="media-browser-sort">
        <option value="estimatedSaveDesc">Est. save high-low</option>
    ...
    const queueCurrent = document.createElement('button');
    queueCurrent.id = 'media-browser-queue-current-folder';
    queueCurrent.type = 'button';
    queueCurrent.className = 'button';
    ...
    const sortSelect = document.getElementById('media-browser-sort');
    sortSelect.value = state.mediaBrowser.sort;
    sortSelect.addEventListener('change', event => {
        state.mediaBrowser.sort = event.target.value || 'estimatedSaveDesc';
        localStorage.setItem('mediaBrowserSort', state.mediaBrowser.sort);
        refreshMediaBrowser();
    });
}

function mediaBrowserMessage(message, kind = '') {
...
function mediaBrowserEstimatedSaveValue(row) {
    return Number(row.totals?.estimatedCleanupSavingsBytes ?? row.media?.estimatedCleanupSavingsBytes ?? 0) || 0;
}

function mediaBrowserSizeValue(row) {
    return Number(row.totals?.originalSizeBytes ?? row.media?.fileSizeBytes ?? 0) || 0;
}

function mediaBrowserActualSavedValue(row) {
    return Number(row.totals?.actualSavedBytes ?? row.media?.actualTotalSavedBytes ?? row.media?.actualSavedBytes ?? 0) || 0;
}

function mediaBrowserNameValue(row) {
    return String(row.name || '').toLocaleLowerCase();
}

function compareMediaBrowserRows(a, b) {
    const sort = state.mediaBrowser.sort || 'estimatedSaveDesc';
    const byName = () => mediaBrowserNameValue(a).localeCompare(mediaBrowserNameValue(b));
    const numeric = (left, right, desc = true) => {
        const result = (left === right) ? byName() : (left - right);
        return desc ? -result : result;
    };
    switch (sort) {
        case 'nameDesc': return -byName();
        case 'estimatedSaveAsc': return numeric(mediaBrowserEstimatedSaveValue(a), mediaBrowserEstimatedSaveValue(b), false);
        case 'sizeDesc': return numeric(mediaBrowserSizeValue(a), mediaBrowserSizeValue(b), true);
        case 'sizeAsc': return numeric(mediaBrowserSizeValue(a), mediaBrowserSizeValue(b), false);
        case 'actualSavedDesc': return numeric(mediaBrowserActualSavedValue(a), mediaBrowserActualSavedValue(b), true);
        case 'estimatedSaveDesc': return numeric(mediaBrowserEstimatedSaveValue(a), mediaBrowserEstimatedSaveValue(b), true);
        case 'nameAsc':
        default: return byName();
    }
}

function renderBrowserLanguage(row) {
    if (row.media) return renderMediaLanguage(row.media);
    const totals = row.totals || {};
    const known = Number(totals.originalLanguageKnownCount || 0);
    const unknown = Number(totals.originalLanguageUnknownCount || 0);
    const distinct = Number(totals.originalLanguageDistinctCount || 0);
    const language = totals.originalLanguage;
    if (!known && unknown) return `Unknown <small>${unknown} file(s)</small>`;
    if (!known && !unknown) return 'Unknown';
    if (distinct <= 1) {
        const unknownSuffix = unknown ? `<small>+ ${unknown} unknown</small>` : '';
        return `${escapeHtml(formatLanguage(language))}${unknownSuffix}`;
    }
    const unknownSuffix = unknown ? `<small>${unknown} unknown</small>` : '';
    return `Mixed <small>${distinct} languages Â· most ${escapeHtml(formatLanguage(language))}</small>${unknownSuffix}`;
}
...
function isMediaBrowserCleanupCandidate(media) {
    if (!media || !media.hasPlan || media.planKind !== 'CleanupOnly') return false;
    if (Number(media.estimatedCleanupSavingsBytes || 0) <= 0) return false;
    return true;
}

async function collectMediaBrowserCleanupCandidates(libraryId, rootPath) {
    ...
    while (queue.length) {
        ...
        seen.add(key);
        const browser = await api(`/api/media/browser?libraryId=${libraryId}&path=${encodeURIComponent(current)}`);
        for (const directory of (browser.directories || [])) queue.push(directory.path || '');
        for (const media of (browser.items || [])) {
            if (isMediaBrowserCleanupCandidate(media)) candidates.push(media);
        }
        mediaBrowserMessage(`Scanning ${seen.size} folder(s), found ${candidates.length} cleanup candidate(s)...`);
    }
    return candidates.sort((a, b) => (Number(b.estimatedCleanupSavingsBytes || 0) - Number(a.estimatedCleanupSavingsBytes || 0)) || String(a.relativePath || '').localeCompare(String(b.relativePath || '')));
}

window.queueMediaBrowserFolderCleanup = async (path = null) => {
    ...
    if (!confirm(`Scan ${scope} and queue cleanup jobs for matching files only?\n\nThis queues existing CleanupOnly plans one file at a time. Originals are untouched; outputs go to staging.`)) return;

    try {
        mediaBrowserMessage(`Scanning ${scope}...`);
        const candidates = await collectMediaBrowserCleanupCandidates(libraryId, rootPath);
        if (!candidates.length) {
            mediaBrowserMessage('No cleanup candidates found.', 'warn');
            alert('No CleanupOnly candidates with estimated savings were found in this folder.');
            return;
        }

        const totalSave = candidates.reduce((sum, media) => sum + Number(media.estimatedCleanupSavingsBytes || 0), 0);
        if (!confirm(`Queue cleanup for ${candidates.length} file(s) in ${scope}?\n\nEstimated saving: ${formatBytes(totalSave)}\n\nThis may take a little while because it queues each file individually.`)) {
            mediaBrowserMessage('Queue cancelled.');
            return;
        }
        ...
        for (let index = 0; index < candidates.length; index++) {
            ...
            try {
                const result = await api(`/api/media/${media.id}/cleanup`, { method: 'POST' });
                if (result?.queued) queued++;
                else if (result?.alreadyQueued) alreadyQueued++;
                else skipped++;
            } catch (error) {
                failed++;
                if (errors.length < 8) errors.push(`${media.relativePath}: ${error.message || error}`);
            }
            if ((index + 1) % 10 === 0 || index + 1 === candidates.length) {
                mediaBrowserMessage(`Queued ${index + 1}/${candidates.length} Â· new ${queued}, already ${alreadyQueued}, skipped ${skipped}, failed ${failed}`);
            }
        }

        const summary = [`Queued cleanup for ${queued} file(s).`, `Already queued: ${alreadyQueued}`, `Skipped: ${skipped}`, `Failed: ${failed}`];
        if (errors.length) summary.push('', 'First errors:', ...errors);
        alert(summary.join('\n'));
        mediaBrowserMessage(`Queue complete Â· new ${queued}, already ${alreadyQueued}, skipped ${skipped}, failed ${failed}`, failed ? 'bad' : 'ok');
        await refreshMediaBrowser();
        await refreshMedia();
        await refreshJobs();
    } catch (error) {
...
async function refreshMediaBrowser() {
    ...
    const sortSelect = document.getElementById('media-browser-sort');
    if (sortSelect && sortSelect.value !== state.mediaBrowser.sort) sortSelect.value = state.mediaBrowser.sort || 'estimatedSaveDesc';
    ...
    if (crumbs) {
        crumbs.innerHTML = (browser.breadcrumbs || []).map(c => `<button class="link-button" onclick="openMediaBrowserPath(${jsStringArg(c.path)})">${escapeHtml(c.name)}</button>`).join(' / ');
    }
