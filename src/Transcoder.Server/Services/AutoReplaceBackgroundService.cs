...
public sealed class AutoReplaceBackgroundService(IServiceScopeFactory scopeFactory, ILogger<AutoReplaceBackgroundService> logger) : BackgroundService
{
    ...
    private async Task RunOnceAsync(CancellationToken cancellationToken)
    {
        ...
        var candidates = await db.MediaItems.AsNoTracking()
            .Include(x => x.Library)
            .Where(x => x.Library != null && (x.Status == MediaStatus.StagedCleaned || x.Status == MediaStatus.Staged || x.Status == MediaStatus.Approved))
        ...
        foreach (var candidate in candidates)
        {
            ...
            if (result.Replaced)
                ...
                logger.LogDebug("Auto-replace skipped media {MediaId}: {Message}", candidate.Id, result.Message);
        }
    }
